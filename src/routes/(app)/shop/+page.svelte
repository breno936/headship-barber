<script lang="ts">
  ...
  function addProduct(prod:any){
    ...
    document.getElementById("alertAdd").style.top = "32px";

    setTimeout(() => {
      document.getElementById("alertAdd").style.top = "-200px";
    }, 2000);
  }
</script>
...
<div class="flex py-32 lg:h-[600px] flex-col items-center md:items-start md:flex-row justify-center overflow-hidden flex-wrap">
  ...
  <div class="w-5/6 sm:w-3/6 md:w-2/6 lg:w-1/6 h-fit mt-6 lg:mt-0 md:ml-6 cardProduct rounded-md overflow-hidden">
      ...
      <div class="h-0 block relative buttonAdd transition-all text-center">
        ...
        {/if}
        <div class="w-full px-4 flex justify-center items-center h-fit relative" style="bottom:-20px;">
            ...
            <button disabled='{product.quantity > 0 ? false : true}' on:click={() => addProduct(product)} class="py-2 px-4 xl:px-7 border border-[#183A5D] text-[#183A5D] rounded-xl font-bold transition-all ml-auto mr-auto block relative">ADICIONAR</button>
